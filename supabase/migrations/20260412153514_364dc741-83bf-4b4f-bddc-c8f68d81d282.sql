...
BEGIN
  ...
  WHERE pcr.partner_id = NEW.partner_id AND pcr.is_active = true;
  ...
  IF total_paid >= 30000 THEN
    ...
    IF partner_record.type = 'PARTNER' THEN
      ...
      VALUES (
        ...
        'A rede atingiu R$ ' || to_char(total_paid, 'FM999G999D00') || ' em créditos pagos. Link de recrutamento gerado automaticamente.',
        'LOW'
      );
    END IF;
  END IF;
