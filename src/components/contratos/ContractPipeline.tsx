...
const columns: ColumnConfig[] = [
  {
    ...
    color: 'bg-primary',
    tooltip: 'Contratos aguardando a assinatura do paciente. O link expira em 72h.',
  },
  {
    title: 'Pendências Gerais',
    status: 'PENDENCIAS_GERAIS',
    icon: <AlertTriangle className="h-4 w-4 sm:h-5 sm:w-5 text-white" />,
    color: 'bg-warning',
    tooltip: 'Contratos com pendências após início do processo de assinatura.',
  },
  {
    title: 'Contrato Pago',
    status: 'PAGO',
    ...
    color: 'bg-success',
    tooltip: 'Contratos finalizados e pagos com sucesso.',
  },
  {
    title: 'Contrato Expirado',
    status: 'EXPIRADO',
    icon: <TimerOff className="h-4 w-4 sm:h-5 sm:w-5 text-white" />,
    color: 'bg-muted-foreground',
    tooltip: 'Contratos expirados (72h sem assinatura). Podem ser regenerados.',
  },
  {
    ...
    tooltip: 'Contratos cancelados pelo banco ou fundo.',
  },
];
...
const ContractPipeline = ({ contracts, onUpdateContracts }: ContractPipelineProps) => {
  ...
  const handleRegenerateContract = (contractId: string) => {
    const updated = contracts.map((c) =>
      c.id === contractId
        ? {
            ...c,
            contract_status: 'AGUARDANDO_ASSINATURA' as ContractStatus,
            signature_link: `https://assinatura.exemplo.com/${c.id}-regenerated`,
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  isMobile ? 'w-72 shrink-0' : 'w-64 shrink-0'
                )}
              >
                {/* Column Header */}
                <div className="mb-3 sm:mb-4 flex items-center gap-2">
                  <div className={cn('rounded-lg p-1.5 sm:p-2', col.color)}>
                    {col.icon}
                  </div>
