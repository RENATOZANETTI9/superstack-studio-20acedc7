...
import type {
  ...
  ContractHistoryItem,
  ScheduledReturn,
  HistoryInteractionType,
  HistoryInteractionStatus,
} from '@/types/contracts';
import { mockHistory, mockScheduledReturns } from '@/types/contracts';
...
const ContractDetailModal = ({ contract, open, onOpenChange, onRegenerate }: ContractDetailModalProps) => {
  const [history, setHistory] = useState<ContractHistoryItem[]>(mockHistory);
  const [scheduledReturns, setScheduledReturns] = useState<ScheduledReturn[]>(mockScheduledReturns);
  ...
  const getExpiredAt = () => {
    const d = new Date(contract.link_generated_at);
    return new Date(d.getTime() + 72 * 3600000).toISOString();
  };
  ...
  const activeReturn = scheduledReturns.find(
    (r) => r.contract_id === contract.id && !r.completed
  );
  ...
  const handleAddHistory = () => {
    ...
    if (newStatus === 'Pediu para retornar' && returnDate && returnTime) {
      const sr: ScheduledReturn = {
        id: `sr-${Date.now()}`,
        contract_id: contract.id,
        date: returnDate,
        time: returnTime,
      ...
      setScheduledReturns((prev) => [...prev, sr]);
    }
  ...
  const handleMarkReturnDone = (returnId: string) => {
    setScheduledReturns((prev) =>
      prev.map((r) => (r.id === returnId ? { ...r, completed: true } : r))
    );
    const item: ContractHistoryItem = {
      id: `h-${Date.now()}`,
      contract_id: contract.id,
            ...
            <TabsContent value="dados" className="p-4 sm:p-6 pt-4 space-y-4 m-0">
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      <p className="text-sm font-medium text-foreground">
                        Este contrato expirou por não ter sido assinado dentro do prazo de 72 horas.
                      </p>
                      <p className="text-xs text-muted-foreground mt-1">
                        Clique em "Regenerar contrato" para gerar um novo link de assinatura.
                      </p>
                    </div>
                  </div>
                  <Button
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                <div>
                  ...
                  <p className="text-sm font-medium text-foreground">{contract.patient_name}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground mb-1">CPF</p>
                  <p className="text-sm font-medium text-foreground">{contract.cpf}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground mb-1">Nº Proposta</p>
                  <p className="text-sm font-medium text-foreground">{contract.proposal_number}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                <div>
                  ...
                  <p className="text-sm font-medium text-foreground">{contract.bank_name}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground mb-1">Status Proposta</p>
                  <p className="text-sm font-medium text-foreground">{contract.proposal_status}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground mb-1">Prazo</p>
                  <p className="text-sm font-medium text-foreground">{contract.term_months}x</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                <div>
                  ...
                  <p className="text-sm font-bold text-primary">{formatCurrency(contract.amount_released)}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground mb-1">Valor Parcela</p>
                  <p className="text-sm font-bold text-primary">{formatCurrency(contract.installment_value)}</p>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  <a
                    ...
                    className="text-sm text-primary hover:text-primary/80 font-medium inline-flex items-center gap-1 transition-colors truncate"
                  >
                    Abrir link
                ...
                <p className="text-[10px] text-muted-foreground mt-1">
                  Gerado em: {formatDate(contract.link_generated_at)}
                </p>
              </div>
            </TabsContent>
              ...
              {activeReturn && (
                <div className="rounded-lg border border-primary/30 bg-primary/5 p-3 space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-sm font-medium text-foreground">
                      Próximo retorno agendado: {activeReturn.date} às {activeReturn.time}
                    </p>
                  </div>
                  ...
                  >
                    <div className="rounded-lg border border-border bg-card p-3 sm:p-4 space-y-3">
                          ...
                          >
                            <div className="rounded-lg border border-primary/20 bg-primary/5 p-3 space-y-2">
                              <p className="text-xs font-medium text-primary flex items-center gap-1">
                                <Clock className="h-3 w-3" /> Agendar Retorno
                              </p>
                      ...
                      <div className="flex gap-2 pt-1">
                        <Button size="sm" className="text-xs h-7" onClick={handleAddHistory}>
                          Salvar
                        </Button>
