  ...
  return (
    <>
                  ...
                  {items.map(c => (
                    <tr key={c.id} className="border-b hover:bg-accent/30">
                      ...
                      {isAdmin && (
                        <td className="py-3">
                          {c.status !== 'PAID' ? (
                            <Button size="sm" variant="outline" className="text-xs gap-1" onClick={() => setConfirmId(c.id)}>
      ...
      <AlertDialog open={!!confirmId} onOpenChange={(open) => !open && setConfirmId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Confirmar pagamento de bonificação</AlertDialogTitle>
            <AlertDialogDescription>
              Tem certeza que deseja marcar esta bonificação como paga? Esta ação não pode ser desfeita.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
